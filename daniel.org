...
(use-package indium
  :ensure t
  :init (defun setup-indium-chome-repl ()
          (interactive)
          (shell-command "chromium-browser --remote-debugging-port=9222 https://localhost:3000")
          (indium-chrome--get-tabs-data "127.0.0.1"
                                        "9222"
                                        #'indium-chrome--connect-to-tab)))
...
(use-package evil
  ...
  :config (progn (evil-mode t)
                 ...
                 (evil-set-initial-state 'fuel-listener-mode 'insert)
